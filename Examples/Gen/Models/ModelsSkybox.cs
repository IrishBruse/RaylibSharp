...
private const int GLSL_VERSION = 100;
    ...
    public static int Example()
    {
        ...
        Camera camera = new();
        camera.Position = (Vector3)new(1.0f,1.0f, 1.0f);    // Camera position
        camera.Target = (Vector3)new(4.0f,1.0f, 4.0f);      // Camera looking at point
        ...
        SetShaderValue(skybox.Materials[0].shader, GetShaderLocation(skybox.Materials[0].shader, "environmentMap"), (int[1]){ MaterialMapIndex.Cubemap }, SHADER_UNIFORM_INT);
        ...
        if (useHDR)
        {
            ...
            skybox.Materials[0].Maps[MaterialMapIndex.Cubemap].texture = GenTextureCubemap(shdrCubemap, panorama, 1024, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);
        ...
        else
        {
            ...
            skybox.Materials[0].Maps[MaterialMapIndex.Cubemap].texture = LoadTextureCubemap(img, CUBEMAP_LAYOUT_AUTO_DETECT);    // CUBEMAP_LAYOUT_PANORAMA
            UnloadImage(img);
        }
            ...
            if (IsFileDropped())
            {
                ...
                if (droppedFiles.Count == 1)         // Only support one file dropped
                {
                    if (IsFileExtension(droppedFiles.Paths[0], ".png;.jpg;.hdr;.bmp;.tga"))
                    {
                        ...
                        UnloadTexture(skybox.Materials[0].Maps[MaterialMapIndex.Cubemap].texture);
                        if (useHDR)
                        {
                        ...
                        else
                        {
                            ...
                            skybox.Materials[0].Maps[MaterialMapIndex.Cubemap].texture = LoadTextureCubemap(img, CUBEMAP_LAYOUT_AUTO_DETECT);
                            UnloadImage(img);
                        }
    ...
    static TextureCubemap GenTextureCubemap(Shader shader, Texture panorama, int size, int format)
    {
        ...
        Matrix fboViews[6] = {
            MatrixLookAt((Vector3)new(0.0f,0.0f, 0.0f), (Vector3)new( 1.0f, 0.0f,  0.0f), (Vector3)new(0.0f,-1.0f,  0.0f)),
            MatrixLookAt((Vector3)new(0.0f,0.0f, 0.0f), (Vector3)new(-1.0f, 0.0f,  0.0f), (Vector3)new(0.0f,-1.0f,  0.0f)),
            MatrixLookAt((Vector3)new(0.0f,0.0f, 0.0f), (Vector3)new( 0.0f, 1.0f,  0.0f), (Vector3)new(0.0f, 0.0f,  1.0f)),
            MatrixLookAt((Vector3)new(0.0f,0.0f, 0.0f), (Vector3)new( 0.0f,-1.0f,  0.0f), (Vector3)new(0.0f, 0.0f, -1.0f)),
            MatrixLookAt((Vector3)new(0.0f,0.0f, 0.0f), (Vector3)new( 0.0f, 0.0f,  1.0f), (Vector3)new(0.0f,-1.0f,  0.0f)),
            MatrixLookAt((Vector3)new(0.0f,0.0f, 0.0f), (Vector3)new( 0.0f, 0.0f, -1.0f), (Vector3)new(0.0f,-1.0f,  0.0f))
        };
