...
private const int MAX_COLORS_COUNT = 23;
    ...
    public static int Example()
    {
        ...
        while (!WindowShouldClose())    // Detect window close button or ESC key
        {
            ...
            if (IsMouseButtonDown(MouseButton.Left) || (GetGestureDetected() == GESTURE_DRAG))
            {
            ...
            if (CheckCollisionPoint(mousePos, btnSaveRec)) btnSaveMouseHover = true;
            ...
            if (mousePos.Y > 50)
            {
                if (IsMouseButtonDown(MouseButton.Right)) DrawCircleLines((int)mousePos.X, (int)mousePos.Y, brushSize, Gray);
            ...
            for (int i = 0; i < MAX_COLORS_COUNT; i++) DrawRectangle(colorsRecs[i], colors[i]);
            DrawRectangleLines(10, 10, 30, 30, LightGray);

            if (colorMouseHover >= 0) DrawRectangle(colorsRecs[colorMouseHover], Fade(White, 0.6f));

            DrawRectangleLines((Rectangle){ colorsRecs[colorSelected].X - 2, colorsRecs[colorSelected].Y - 2,
